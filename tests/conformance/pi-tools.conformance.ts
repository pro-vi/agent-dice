 ...
 * execute() directly (as the model would). Proves register_dice registers/validates,
...
export const checks: Check[] = [
  {
    name: "register_dice: agent registers a slot from structured params",
    fn: () =>
      withTempBase(async () => {
        const tools = captureTools();
        assert(typeof tools.register_dice?.execute === "function", "register_dice tool registered");
        const r = await tools.register_dice.execute(
          "1",
  ...
  {
    name: "register_dice: target > die is rejected, not persisted",
    fn: () =>
      withTempBase(async () => {
        const tools = captureTools();
        const r = await tools.register_dice.execute("1", { name: "bad", message: "m", die: 6, target: 20 }, undefined, undefined, ctx());
        assert(/exceeds die size/.test(out(r)), `result: ${out(r)}`);
        assertEqual(await getSlot("bad"), null, "invalid slot not persisted");
      }),
  },
  {
    name: "register_dice: an invalid slot name fails gracefully (no throw)",
    fn: () =>
      withTempBase(async () => {
        const tools = captureTools();
        const r = await tools.register_dice.execute("1", { name: "bad name", message: "m" }, undefined, undefined, ctx());
  ...
  },
  {
    ...
    fn: () =>
      withTempBase(async () => {
        const tools = captureTools();
        await tools.register_dice.execute("1", { name: "rf", message: "m", type: "single", die: 20, target: 1 }, undefined, undefined, ctx());
